/**
 * Amer Fort Explorer Interactive Script
 * Handles Journey bookmarking, lightbox modal, and scroll-to-top.
 */

document.addEventListener('DOMContentLoaded', () => {
  // Lightbox Modal Functionality
  const modal = document.getElementById('amer-modal');
  const modalImg = document.getElementById('amer-modal-img');
  const modalCaption = document.getElementById('amer-modal-caption');
  const modalOverlay = document.getElementById('amer-modal-overlay');
  const modalClose = document.getElementById('amer-modal-close');
  const galleryItems = document.querySelectorAll('.gallery-item');

  function openModal(imgSrc, captionText) {
    if (!modal || !modalImg) return;
    modalImg.src = imgSrc;
    modalImg.alt = captionText || 'Amer Fort Gallery Image';
  ...
  if (bookmarkBtn) {
    const bookmarkId = bookmarkBtn.getAttribute('data-bookmark-id') || 'amer-fort-explorer-main';
