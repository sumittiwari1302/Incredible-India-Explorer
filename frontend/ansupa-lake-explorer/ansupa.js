...
function renderStats() {
    ...
    if (!grid || typeof ANSUPA_INFO === 'undefined') return;

    grid.innerHTML = ANSUPA_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    const oxbowEl = document.getElementById('eco-oxbow');
    const floraEl = document.getElementById('eco-flora');
    ...
    if (oxbowEl) oxbowEl.innerHTML = `<strong>Oxbow Hydrology & River Connection:</strong> ${ECOLOGY_HYDROLOGY.oxbowHydrology}`;
    if (floraEl) floraEl.innerHTML = `<strong>Aquatic Macrophytes & Lotus Blooms:</strong> ${ECOLOGY_HYDROLOGY.aquaticFlora}`;
    if (statusEl) statusEl.innerHTML = `<strong>Conservation Status:</strong> ${ECOLOGY_HYDROLOGY.conservationStatus}`;
}

function renderSpecies() {
    const grid = document.getElementById('species-grid');
    if (!grid || typeof BIRD_FLORA_SPECIES === 'undefined') return;

    grid.innerHTML = BIRD_FLORA_SPECIES.map(
        sp => `
        <div class="species-card">
            ...
            <div class="species-card-body">
                ...
                <div class="bird-meta">
                    ...
                    <span>📏 ${sp.wingspan || sp.habitat}</span>
                </div>
            </div>
        </div>
    `
