/**
 * Ashtamudi Lake Explorer — Logic & DOM Interactivity
 */
...
function renderStats() {
    ...
    if (!container || typeof ASHTAMUDI_LAKE_INFO === 'undefined') return;

    container.innerHTML = ASHTAMUDI_LAKE_INFO.quickStats.map(stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    setElText('eco-backwaters', ECOLOGY_HYDROLOGY.backwaters);
    ...
    setElText('eco-birdlife', ECOLOGY_HYDROLOGY.birdlife);
}
    ...
    container.innerHTML = MAP_HOTSPOTS.map(spot => `
        <div class="hotspot-card">
            <div class="hotspot-content">
                ...
                <div style="font-size: 0.85rem; color: var(--ashtamudi-muted); margin-bottom: 0.5rem;">${spot.type} (${spot.lat}°N, ${spot.lng}°E)</div>
    ...
    container.innerHTML = GALLERY_IMAGES.map(img => `
        <div class="gallery-card">
            ...
            <div class="gallery-caption">
                ...
                <span style="font-size: 0.75rem; display: block; color: var(--ashtamudi-muted); margin-top: 0.25rem;">Category: ${img.category}</span>
            </div>
        </div>
    `).join('');
}
