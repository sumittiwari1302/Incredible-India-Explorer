/**
 * Veerapandiya Kattabomman Explorer - Interactive Script
...
function activateTab(targetTab) {
    const tabs = document.querySelectorAll('.kbm-tab');
    const sections = document.querySelectorAll('.kbm-section');
    if (!tabs.length || !sections.length) return;

    tabs.forEach(t => t.classList.remove('active'));
    sections.forEach(s => s.classList.remove('active'));

    const tab = document.querySelector(`.kbm-tab[data-tab="${targetTab}"]`);
...
function initTabNavigation() {
    ...
    if (hash) {
        const valid = ['biography', 'timeline', 'resistance', 'significance', 'gallery', 'references'];
...
function initCountUp() {
    const counters = document.querySelectorAll('.kbm-count');
