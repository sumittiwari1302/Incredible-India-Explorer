/**
 * Rashtriya Khel Protsahan Puruskar Explorer - Interactive Script
...
function activateTab(targetTab) {
    const tabs = document.querySelectorAll('.rkpp-tab');
    const sections = document.querySelectorAll('.rkpp-section');
    if (!tabs.length || !sections.length) return;

    tabs.forEach(t => t.classList.remove('active'));
    sections.forEach(s => s.classList.remove('active'));

    const tab = document.querySelector(`.rkpp-tab[data-tab="${targetTab}"]`);
...
function initTabNavigation() {
    document.querySelectorAll('.rkpp-tab').forEach(tab => {
...
function initScrollAnimations() {
    ...
    const targets = document.querySelectorAll('.eligibility-item, .category-card, .selection-step, .awardee-card, .gallery-item, .rkpp-timeline-card, .rkpp-banner, .rkpp-content-card');
    targets.forEach(el => {
...
function initFooterTabs() {
    document.querySelectorAll('.rkpp-footer-tab').forEach(link => {
...
function initCardTilt() {
    ...
    cards.forEach(card => {
        card.addEventListener('mousemove', (e) => {
            ...
            const rx = (0.5 - py) * 8;
            const ry = (px - 0.5) * 10;
            card.classList.add('rkpp-card-tilt');
 ...
 */
function initTyping() {
    const el = document.querySelector('.rkpp-type[data-words]');
    if (!el) return;
...
function initHeroParallax() {
    const bg = document.querySelector('.rkpp-hero-bg');
    if (!bg) return;
...
function initCountUp() {
    const counters = document.querySelectorAll('.rkpp-count');
    if (!counters.length) return;

    const duration = 1600;
    const hero = document.querySelector('.rkpp-hero');
