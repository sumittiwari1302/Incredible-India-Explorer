...
function renderStats() {
    ...
    if (!grid || typeof BHIMLAT_INFO === 'undefined') return;

    grid.innerHTML = BHIMLAT_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderHeightComparison() {
    ...
    grid.innerHTML = HEIGHT_COMPARISONS.map(item => {
        ...
        const isBhimlat = item.name.includes('Bhimlat');
        return `
        <div class="height-card ${isBhimlat ? 'highlight' : ''}">
...
function renderAttractions() {
    ...
    grid.innerHTML = NEARBY_ATTRACTIONS.map(
        ...
        <div class="attraction-card">
            <h3>📍 ${a.name}</h3>
            <span class="dist-tag">${a.distance} from Bhimlat</span>
            <p>${a.description}</p>
        </div>
    `
    ).join('');
