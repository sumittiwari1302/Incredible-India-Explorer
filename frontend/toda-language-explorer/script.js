/**
 * Interactive Script for Toda Language Explorer (Nilgiri Hills, Tamil Nadu)
 */

document.addEventListener('DOMContentLoaded', () => {
    const data = window.TODA_DATA;
    if (!data) {
        console.error("Toda dataset missing!");
        return;
    }
    ...
    if (themeBtn) {
        const savedTheme = localStorage.getItem('toda_theme') || 'dark';
        if (savedTheme === 'light') {
        ...
        themeBtn.addEventListener('click', () => {
            ...
            localStorage.setItem('toda_theme', isLight ? 'light' : 'dark');
        });
    }
    ...
    function playAudio(text, lang = 'ta-IN') {
        if (!('speechSynthesis' in window)) {
        ...
        const bestVoice = voices.find(v => v.lang.startsWith('ta') || v.lang.startsWith('en-IN') || v.lang.startsWith('hi'));
        if (bestVoice) {
        ...
        if (regData && mapTitle && mapBadge && mapDesc) {
            ...
            mapDesc.innerHTML = `
                ...
                <div style="font-size: 0.88rem; color: var(--text-muted); background: rgba(255,255,255,0.03); padding: 12px 16px; border-radius: 8px; border-left: 3px solid ${regData.color};">
                    <strong style="color: var(--text-primary);">Key Settlements (Munds):</strong> ${regData.subRegions.join(', ')}
                </div>
            `;
        }
    }
    ...
    function renderGreetings(filter = '') {
        ...
        greetingsGrid.innerHTML = list.map(g => `
            <div class="greeting-card">
                <div>
                    <span class="region-badge" style="background: rgba(168, 70, 47, 0.2); color: var(--accent-rust-bright); margin-bottom: 12px; display: inline-block;">${g.category}</span>
        ...
        cultureGrid.innerHTML = data.culturalHeritage.map(c => `
            <div class="culture-card">
                <div class="culture-card-body">
                    ...
                    <span class="region-badge" style="background: rgba(201, 154, 58, 0.2); color: var(--accent-gold-bright); margin-bottom: 12px; display: inline-block;">${c.category}</span>
        ...
        if (currentQ >= questions.length) {
            quizContainer.innerHTML = `
                <div style="text-align: center; padding: 24px;">
                    ...
                    <h3 style="font-size: 2rem; margin-bottom: 10px; font-weight: 700;">Quiz Completed!</h3>
                    ...
                    <p style="color: var(--text-secondary); margin-bottom: 28px; max-width: 500px; margin-left: auto; margin-right: auto;">
                        ${score === questions.length ? 'Isk vaa! You have mastered the Toda language, phonology, and Nilgiri pastoral heritage!' : 'Well done! Explore the vocabulary flipper and phonetics sections to reinforce your knowledge.'}
                    </p>
        ...
        optBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (selected === q.correct) {
                    ...
                    feedback.style.background = 'rgba(74, 103, 65, 0.25)';
                    feedback.style.border = '1px solid #688e5d';
                    feedback.style.color = '#92c585';
                    feedback.innerHTML = `✓ Correct! ${q.explanation}`;
                } else {
                    btn.classList.add('wrong');
                    optBtns[q.correct].classList.add('correct');
                    feedback.style.display = 'block';
                    feedback.style.background = 'rgba(168, 70, 47, 0.25)';
                    feedback.style.border = '1px solid #a8462f';
