...
function renderStats() {
    ...
    if (!grid || typeof TRAVANCORE_FROG_INFO === 'undefined') return;

    grid.innerHTML = TRAVANCORE_FROG_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    if (distEl) distEl.innerHTML = `<strong>Distribution Range:</strong> ${ECOLOGY_BEHAVIOUR.distribution}`;
    if (habEl) habEl.innerHTML = `<strong>Rainforest Habitat:</strong> ${ECOLOGY_BEHAVIOUR.habitat}`;
    if (morphEl) morphEl.innerHTML = `<strong>Morphology & Webbing:</strong> ${ECOLOGY_BEHAVIOUR.morphology}`;
    if (behEl) behEl.innerHTML = `<strong>Arboreal Parachuting Behaviour:</strong> ${ECOLOGY_BEHAVIOUR.behaviour}`;
