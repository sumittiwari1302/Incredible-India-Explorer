...
function renderStats() {
    ...
    if (!grid || typeof SULTANPUR_INFO === 'undefined') return;

    grid.innerHTML = SULTANPUR_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    if (hydroEl) hydroEl.innerHTML = `<strong>Hydrology & Acacia Roosting Mounds:</strong> ${ECOLOGY_HYDROLOGY.hydrology}`;
    if (historyEl) historyEl.innerHTML = `<strong>History & Salim Ali Conservation Effort:</strong> ${ECOLOGY_HYDROLOGY.history}`;
