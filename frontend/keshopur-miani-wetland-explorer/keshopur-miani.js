...
function renderStats() {
    ...
    if (!grid || typeof KESHOPUR_MIANI_INFO === 'undefined') return;

    grid.innerHTML = KESHOPUR_MIANI_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    const wetlandEl = document.getElementById('eco-wetland');
    ...
    if (wetlandEl) wetlandEl.innerHTML = `<strong>Wetland Ecology:</strong> ${ECOLOGY_HYDROLOGY.wetlandEcology}`;
    if (fisheriesEl) fisheriesEl.innerHTML = `<strong>Community Fisheries:</strong> ${ECOLOGY_HYDROLOGY.communityFisheries}`;
