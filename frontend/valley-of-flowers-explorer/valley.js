...
function renderStats() {
    ...
    if (!grid || typeof VALLEY_INFO === 'undefined') return;

    grid.innerHTML = VALLEY_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    const historyEl = document.getElementById('eco-history');
    ...
    if (hydroEl) hydroEl.innerHTML = `<strong>Pushpawati River & Alpine Hydrology:</strong> ${ECOLOGY_HYDROLOGY.hydrology}`;
    if (historyEl) historyEl.innerHTML = `<strong>History & Frank Smythe Discovery:</strong> ${ECOLOGY_HYDROLOGY.history}`;
    if (statusEl) statusEl.innerHTML = `<strong>Conservation Status:</strong> ${ECOLOGY_HYDROLOGY.conservationStatus}`;
}

function renderSpecies() {
    const grid = document.getElementById('species-grid');
    if (!grid || typeof FLORA_FAUNA_SPECIES === 'undefined') return;

    grid.innerHTML = FLORA_FAUNA_SPECIES.map(
        sp => `
        <div class="species-card">
            ...
            <div class="species-card-body">
                ...
                <div class="bird-meta">
                    ...
                    <span>🏞️ ${sp.habitat || sp.diet}</span>
                </div>
            </div>
        </div>
    `
