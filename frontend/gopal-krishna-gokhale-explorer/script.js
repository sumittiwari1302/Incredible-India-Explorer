/**
 * Gopal Krishna Gokhale Explorer - Interactive Script
...
function activateTab(targetTab) {
    const tabs = document.querySelectorAll('.gokhale-tab');
    const sections = document.querySelectorAll('.gokhale-section');
    if (!tabs.length || !sections.length) return;

    tabs.forEach(t => t.classList.remove('active'));
    sections.forEach(s => s.classList.remove('active'));

    const tab = document.querySelector(`.gokhale-tab[data-tab="${targetTab}"]`);
...
function initTabNavigation() {
    ...
    if (hash) {
        const valid = ['biography', 'timeline', 'congress', 'social-reforms', 'gandhi-influence', 'legacy', 'references'];
...
function initCountUp() {
    const counters = document.querySelectorAll('.gokhale-count');
