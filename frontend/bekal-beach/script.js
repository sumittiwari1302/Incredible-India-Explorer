/**
 * Bekal Beach Profile Interactive Features
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initMap() {
        if (!window.L) return;
        const mapEl = document.getElementById('bekal-map');
        if (!mapEl) return;

        const map = L.map('bekal-map', { scrollWheelZoom: false }).setView([12.3926, 75.0336], 13);
        ...
        L.marker([12.3926, 75.0336])
            .addTo(map)
            .bindPopup('<strong>Bekal Beach & Bekal Fort</strong><br>Kasaragod, Kerala')
            .openPopup();

        const nearby = [
            { name: 'Kappil Beach & Estuary', coords: [12.4410, 75.0020], note: '~6 km North' },
            { name: 'Chandragiri Fort', coords: [12.4830, 74.9950], note: '~18 km North' },
            { name: 'Valiyaparamba Backwaters', coords: [12.1640, 75.1480], note: '~30 km South' }
        ];
    ...
    galleryItems.forEach(item => {
        const openLightbox = () => {
            ...
            if (img && modal && modalImg) {
                ...
                modalTitle.textContent = title ? title.textContent : 'Bekal Beach Visual';
