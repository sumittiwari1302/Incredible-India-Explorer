/**
 * Anjuna Beach Profile Interactive Features
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    galleryItems.forEach(item => {
        const openLightbox = () => {
            ...
            if (img && modal && modalImg) {
                ...
                modalTitle.textContent = title ? title.textContent : 'Anjuna Beach Visual';
    ...
    function initMap() {
        if (!window.L) return;
        const mapEl = document.getElementById('anjuna-map');
        if (!mapEl) return;

        const map = L.map('anjuna-map', { scrollWheelZoom: false }).setView([15.5749, 73.7410], 13);
        ...
        L.marker([15.5749, 73.7410])
            .addTo(map)
            .bindPopup('<strong>Anjuna Beach</strong><br>North Goa, India')
            .openPopup();

        const nearby = [
            { name: 'Vagator Beach', coords: [15.5839, 73.7367], note: '~2 km north' },
            { name: 'Baga Beach', coords: [15.5585, 73.7520], note: '~5 km south' },
            { name: 'Chapora Fort', coords: [15.5967, 73.7367], note: '~1 km north' },
            { name: 'Calangute Beach', coords: [15.5449, 73.7553], note: '~6 km south' }
        ];
