/**
 * Interactive Script for Kurukh Language Explorer (Chota Nagpur & Beyond)
 */

document.addEventListener('DOMContentLoaded', () => {
    const data = window.KURUKH_DATA;
    if (!data) {
        console.error("Kurukh dataset missing!");
        return;
    }
    ...
    if (themeBtn) {
        const savedTheme = localStorage.getItem('kurukh_theme') || 'dark';
        if (savedTheme === 'light') {
        ...
        themeBtn.addEventListener('click', () => {
            ...
            localStorage.setItem('kurukh_theme', isLight ? 'light' : 'dark');
        });
    }
    ...
    function playAudio(text, lang = 'hi-IN') {
        ...
        const bestVoice = voices.find(v => v.lang.startsWith('hi') || v.lang.startsWith('en-IN') || v.lang.startsWith('or') || v.lang.startsWith('bn'));
        if (bestVoice) {
        ...
        if (regData && mapTitle && mapBadge && mapDesc) {
            ...
            mapDesc.innerHTML = `
                ...
                <div style="font-size: 0.88rem; color: var(--text-muted); background: rgba(255,255,255,0.03); padding: 12px 16px; border-radius: 8px; border-left: 3px solid ${regData.color};">
                    <strong style="color: var(--text-primary);">Key Districts / Hubs:</strong> ${regData.subRegions.join(', ')}
                </div>
            `;
        }
    }
    ...
    function renderGreetings(filter = '') {
        ...
        greetingsGrid.innerHTML = list.map(g => `
            <div class="greeting-card">
                <div>
                    <span class="region-badge" style="background: rgba(34, 197, 94, 0.2); color: var(--accent-green-bright); margin-bottom: 12px; display: inline-block;">${g.category}</span>
    ...
    function renderVocabTable(filter = '') {
        ...
        vocabTableBody.innerHTML = list.map(v => `
            <tr>
                ...
                <td style="font-family: var(--font-mono); color: var(--accent-gold); font-weight: 600;">${v.transliteration}</td>
        ...
        cultureGrid.innerHTML = data.culturalHeritage.map(c => `
            <div class="culture-card">
                <div class="culture-card-body">
                    ...
                    <span class="region-badge" style="background: rgba(34, 197, 94, 0.2); color: var(--accent-green-bright); margin-bottom: 12px; display: inline-block;">${c.category}</span>
        ...
        if (currentQ >= questions.length) {
            quizContainer.innerHTML = `
                <div style="text-align: center; padding: 24px;">
                    ...
                    <p style="font-size: 1.3rem; color: var(--accent-green-bright); margin-bottom: 20px; font-weight: 700;">Your Score: ${score} / ${questions.length}</p>
                    <p style="color: var(--text-secondary); margin-bottom: 28px; max-width: 500px; margin-left: auto; margin-right: auto;">
                        ${score === questions.length ? 'Jai Dharme! You have demonstrated exceptional knowledge of Kurukh language, Tolong Siki script, and Oraon heritage!' : 'Great effort! Revisit the visualizer and greetings soundboard to master every concept.'}
                    </p>
