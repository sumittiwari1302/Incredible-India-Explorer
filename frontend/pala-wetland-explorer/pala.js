...
function renderStats() {
    ...
    if (!grid || typeof PALA_INFO === 'undefined') return;

    grid.innerHTML = PALA_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    const legendEl = document.getElementById('eco-legend');
    ...
    if (hydroEl) hydroEl.innerHTML = `<strong>Freshwater Ecology:</strong> ${ECOLOGY_HYDROLOGY.freshwaterEcology}`;
    if (legendEl) legendEl.innerHTML = `<strong>Mara Folk Legend:</strong> ${ECOLOGY_HYDROLOGY.legend}`;
