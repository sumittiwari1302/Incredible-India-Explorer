/**
 * Hawa Mahal Palace of Winds Explorer Interactive Engine
 * Handles Journey bookmarking, lightbox modal, scroll-to-top, and keyboard navigation.
 */

document.addEventListener('DOMContentLoaded', () => {
  // Gallery Modal Lightbox
  const modal = document.getElementById('hawa-modal');
  const modalImg = document.getElementById('hawa-modal-img');
  const modalCaption = document.getElementById('hawa-modal-caption');
  const modalOverlay = document.getElementById('hawa-modal-overlay');
  const modalClose = document.getElementById('hawa-modal-close');
  ...
  if (bookmarkBtn) {
    const bookmarkId = bookmarkBtn.getAttribute('data-bookmark-id') || 'hawa-mahal-monument-main';
