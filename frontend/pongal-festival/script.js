...
(function () {
    ...
    function init() {
        renderPongalTimeline();
        renderKolamTraditions();
        renderFoodExplorer();
        renderPongalGallery();
        setupTabs();
        setupThemeToggle();
    ...
    function setupBookmark() {
        ...
        if (!btn) return;
        const id = 'festival-pongal';
        const updateBtn = () => { btn.textContent = window.Journey && window.Journey.isSaved(id) ? '✅ Saved to Journey' : '🔖 Bookmark to My Journey'; };
        updateBtn();
        btn.addEventListener('click', () => {
            if (window.Journey) {
                window.Journey.toggle({ id, explorerPage: 'frontend/pongal-festival/index.html', title: 'Pongal Festival', thumbnail: 'https://placehold.co/100/FFCA28/2A1B12', category: 'festivals' });
                updateBtn();
            }
        });
    }
    ...
    function setupJourneyIntegration() {
        if (window.Journey && typeof window.Journey.registerSearchItems === 'function') {
            window.Journey.registerSearchItems('frontend/pongal-festival/index.html', [
                { id: 'festival-pongal', title: 'Pongal', description: 'Tamil Nadu\'s four-day harvest thanksgiving festival.', link: '#' }
            ]);
        }
    }
