...
function renderStats() {
    ...
    if (!grid || typeof KEOTI_INFO === 'undefined') return;

    grid.innerHTML = KEOTI_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderHeightComparison() {
    ...
    grid.innerHTML = HEIGHT_COMPARISONS.map(item => {
        ...
        const isKeoti = item.name.includes('Keoti');
        return `
        <div class="height-card ${isKeoti ? 'highlight' : ''}">
...
function renderAttractions() {
    ...
    grid.innerHTML = NEARBY_ATTRACTIONS.map(
        ...
        <div class="attraction-card">
            <h3>📍 ${a.name}</h3>
            <span class="dist-tag">${a.distance} from Keoti</span>
            <p>${a.description}</p>
        </div>
    `
    ).join('');
