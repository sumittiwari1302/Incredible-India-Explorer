...
function renderStats() {
    ...
    if (!grid || typeof SHALLABUG_INFO === 'undefined') return;

    grid.innerHTML = SHALLABUG_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    const marshEl = document.getElementById('eco-marsh');
    const livelihoodEl = document.getElementById('eco-livelihood');
    ...
    if (marshEl) marshEl.innerHTML = `<strong>Marsh Ecology & Reed Beds:</strong> ${ECOLOGY_HYDROLOGY.marshEcology}`;
    if (livelihoodEl) livelihoodEl.innerHTML = `<strong>Local Community & Wicker Craft:</strong> ${ECOLOGY_HYDROLOGY.communityLivelihood}`;
