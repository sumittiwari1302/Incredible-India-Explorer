...
function renderStats() {
    ...
    if (!grid || typeof CHAMPANER_INFO === 'undefined') return;

    grid.innerHTML = CHAMPANER_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderMonuments() {
    ...
    if (!grid || typeof MONUMENTAL_STRUCTURES === 'undefined') return;

    grid.innerHTML = MONUMENTAL_STRUCTURES.map(
        m => `
        <div class="monument-card">
            ...
            <p><strong>Builder / Period:</strong> ${m.builtBy}</p>
            <p>${m.description}</p>
        </div>
    `
    ).join('');
