/**
 * Bhojpuri Cinema Explorer - Interactive Logic
 * Handles modal popups for gallery items, smooth scroll anchors, and UI state management.
 */

document.addEventListener('DOMContentLoaded', () => {
  // Gallery Modal Elements
  const modal = document.getElementById('bhojpuri-cinema-modal');
  const modalClose = document.getElementById('bhojpuri-cinema-modal-close');
  const modalTitle = document.getElementById('modal-title');
  const modalHeading = document.getElementById('modal-heading');
  const modalDescription = document.getElementById('modal-description');
  const galleryItems = document.querySelectorAll('.bhojpuri-cinema-gallery-item');
