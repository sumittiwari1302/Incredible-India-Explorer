...
function renderStats() {
    ...
    if (!grid || typeof VELLODE_INFO === 'undefined') return;

    grid.innerHTML = VELLODE_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    if (hydroEl) hydroEl.innerHTML = `<strong>Hydrology & Tree Mounds:</strong> ${ECOLOGY_HYDROLOGY.hydrology}`;
    if (historyEl) historyEl.innerHTML = `<strong>History & Sanctuary Origin:</strong> ${ECOLOGY_HYDROLOGY.history}`;
