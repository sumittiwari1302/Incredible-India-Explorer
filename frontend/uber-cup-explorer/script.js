/**
 * Uber Cup Explorer - Interactive Script (#2540)
...
function activateTab(targetTab) {
    ...
    var sections = document.querySelectorAll('.uc-section');
    if (!tabs.length || !sections.length) return;

    tabs.forEach(function (t) { t.classList.remove('active'); });
    sections.forEach(function (s) { s.classList.remove('active'); });

    var tab = document.querySelector('.uc-tab[data-tab="' + targetTab + '"]');
...
function initFooterTabs() {
    document.querySelectorAll('.uc-footer-tab').forEach(function (link) {
 ...
 */
function initTyping() {
    var el = document.querySelector('.uc-type[data-words]');
    if (!el) return;
...
function renderPlayers() {
    ...
    data.players.forEach(function (p) {
        ...
        meta.textContent = p.role + ' \u00B7 Uber Cups: ' + (p.uberCups || '-');
...
/**
 * Build one result row element for a tournament entry
 */
function buildResultItem(t) {
    ...
    var india = document.createElement('p');
    india.className = 'india-result';
    india.innerHTML = '<strong>India:</strong> ' + t.india;
    item.appendChild(india);
...
/**
 * Render India's major results into #results-list
 */
...
function renderTournamentTimeline() {
    ...
    data.tournaments.forEach(function (t, i) {
        ...
        actionHint.setAttribute('aria-label', 'View details of the ' + t.year + ' Uber Cup');
...
/**
 * Initialize the interactive player & tournament timeline:
