/**
 * Mahabalipuram Explorer Interactive Script
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const modalImg = document.getElementById('mb-modal-img');
  const modalCaption = document.getElementById('mb-modal-caption');
  const modalOverlay = document.getElementById('mb-modal-overlay');
  const modalClose = document.getElementById('mb-modal-close');
  const galleryItems = document.querySelectorAll('.gallery-item');

  function openModal(imgSrc, captionText) {
    if (!modal || !modalImg) return;
    modalImg.src = imgSrc;
    modalImg.alt = captionText || 'Mahabalipuram Gallery Image';
  ...
  if (bookmarkBtn) {
    const bookmarkId = bookmarkBtn.getAttribute('data-bookmark-id') || 'mahabalipuram-explorer-main';
