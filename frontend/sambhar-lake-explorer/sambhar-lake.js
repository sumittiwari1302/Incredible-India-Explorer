/**
 * Sambhar Lake Explorer — Logic & DOM Interactivity
 */
...
function renderStats() {
    ...
    if (!container || typeof SAMBHAR_LAKE_INFO === 'undefined') return;

    container.innerHTML = SAMBHAR_LAKE_INFO.quickStats.map(stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    setElText('eco-salt', ECOLOGY_HYDROLOGY.saltProduction);
    setElText('eco-flamingos', ECOLOGY_HYDROLOGY.flamingos);
    ...
    setElText('eco-wildlife', ECOLOGY_HYDROLOGY.wildlife);
}
    ...
    container.innerHTML = MAP_HOTSPOTS.map(spot => `
        <div class="hotspot-card">
            <div class="hotspot-content">
                <h3 style="margin: 0 0 0.5rem; color: #f472b6;">📍 ${spot.title}</h3>
                <div style="font-size: 0.85rem; color: var(--sambhar-muted); margin-bottom: 0.5rem;">${spot.type} (${spot.lat}°N, ${spot.lng}°E)</div>
    ...
    container.innerHTML = GALLERY_IMAGES.map(img => `
        <div class="gallery-card">
            ...
            <div class="gallery-caption">
                ...
                <span style="font-size: 0.75rem; display: block; color: var(--sambhar-muted); margin-top: 0.25rem;">Category: ${img.category}</span>
            </div>
        </div>
    `).join('');
}
