/**
 * Interactive Script for Malvi Language Explorer (Heart of Malwa Plateau)
 */

document.addEventListener('DOMContentLoaded', () => {
    const data = window.MALVI_DATA;
    if (!data) {
        console.error("Malvi dataset missing!");
        return;
    }
    ...
    if (themeBtn) {
        const savedTheme = localStorage.getItem('malvi_theme') || 'dark';
        if (savedTheme === 'light') {
        ...
        themeBtn.addEventListener('click', () => {
            ...
            localStorage.setItem('malvi_theme', isLight ? 'light' : 'dark');
        });
    }
    ...
    function playAudio(text, lang = 'hi-IN') {
        ...
        const bestVoice = voices.find(v => v.lang.startsWith('hi') || v.lang.startsWith('mr') || v.lang.startsWith('gu') || v.lang.startsWith('en-IN'));
        if (bestVoice) {
        ...
        if (regData && mapTitle && mapBadge && mapDesc) {
            ...
            mapDesc.innerHTML = `
                ...
                <div style="font-size: 0.88rem; color: var(--text-muted); background: rgba(255,255,255,0.03); padding: 12px 16px; border-radius: 8px; border-left: 3px solid ${regData.color};">
                    <strong style="color: var(--text-primary);">Key Districts / Cultural Hubs:</strong> ${regData.subRegions.join(', ')}
                </div>
            `;
        }
    }
    ...
    function renderGreetings(filter = '') {
        ...
        greetingsGrid.innerHTML = list.map(g => `
            <div class="greeting-card">
                <div>
                    <span class="region-badge" style="background: rgba(245, 158, 11, 0.2); color: var(--accent-gold-bright); margin-bottom: 12px; display: inline-block;">${g.category}</span>
        ...
        cultureGrid.innerHTML = data.culturalHeritage.map(c => `
            <div class="culture-card">
                <div class="culture-card-body">
                    ...
                    <span class="region-badge" style="background: rgba(245, 158, 11, 0.2); color: var(--accent-gold-bright); margin-bottom: 12px; display: inline-block;">${c.category}</span>
        ...
        if (currentQ >= questions.length) {
            quizContainer.innerHTML = `
                <div style="text-align: center; padding: 24px;">
                    ...
                    <p style="font-size: 1.3rem; color: var(--accent-gold-bright); margin-bottom: 20px; font-weight: 700;">Your Score: ${score} / ${questions.length}</p>
                    <p style="color: var(--text-secondary); margin-bottom: 28px; max-width: 500px; margin-left: auto; margin-right: auto;">
                        ${score === questions.length ? 'Ram Ram Sa! You have demonstrated exceptional knowledge of the Malvi language, Malwa culture, and Maach theatre!' : 'Great effort! Revisit the vocabulary visualizer and cultural traditions to master every concept.'}
                    </p>
        ...
        optBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (selected === q.correct) {
                    ...
                    feedback.style.background = 'rgba(22, 163, 74, 0.25)';
                    ...
                    feedback.style.display = 'block';
                    feedback.style.background = 'rgba(234, 88, 12, 0.25)';
                    feedback.style.border = '1px solid #ea580c';
