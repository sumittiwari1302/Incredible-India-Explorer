/**
 * Baga Beach Profile Interactive Features
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    galleryItems.forEach(item => {
        const openLightbox = () => {
            ...
            if (img && modal && modalImg) {
                ...
                modalTitle.textContent = title ? title.textContent : 'Baga Beach Visual';
    ...
    function initMap() {
        if (!window.L) return;
        const mapEl = document.getElementById('baga-map');
        if (!mapEl) return;

        const map = L.map('baga-map', { scrollWheelZoom: false }).setView([15.5585, 73.7520], 13);
        ...
        L.marker([15.5585, 73.7520])
            .addTo(map)
            .bindPopup('<strong>Baga Beach</strong><br>North Goa, India')
            .openPopup();

        const nearby = [
            { name: 'Calangute Beach', coords: [15.5439, 73.7553], note: '~2 km south' },
            { name: 'Anjuna Beach', coords: [15.5749, 73.7410], note: '~5 km north' },
            { name: 'Chapora Fort', coords: [15.5967, 73.7367], note: '~8 km north' },
            { name: 'Fort Aguada', coords: [15.4919, 73.7758], note: '~10 km south' }
        ];
