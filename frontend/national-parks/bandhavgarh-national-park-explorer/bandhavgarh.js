/**
 * Bandhavgarh National Park Explorer — Interactive Logic
...
(function() {
  ...
  function renderQuickStats() {
    ...
    if (!container || typeof BANDHAVGARH_INFO === 'undefined') return;

    let html = '';
    BANDHAVGARH_INFO.quickStats.forEach(function(st) {
      html += `
      ...
      html += `
        <div class="timeline-item">
          ...
          <div class="glass-card">
            <div style="font-weight:800; font-size:1.2rem; color:var(--bandhavgarh-accent-bright); margin-bottom:0.4rem;">${item.year}</div>
            <h4 style="margin-bottom:0.5rem;">${item.title}</h4>
            <p style="font-size:0.92rem; color:var(--bandhavgarh-muted); line-height:1.6;">${item.description}</p>
          </div>
        </div>`;
    });
    container.innerHTML = html;
  ...
  function renderFort() {
    const container = document.getElementById('fort-box');
    if (!container || typeof BANDHAVGARH_FORT === 'undefined') return;

    const highlightsHtml = BANDHAVGARH_FORT.keyHighlights.map(i => `<li>${i}</li>`).join('');

    container.innerHTML = `
      <h3 style="font-size:1.5rem; color:var(--bandhavgarh-accent-bright); margin-bottom:0.5rem;">🏰 ${BANDHAVGARH_FORT.title}</h3>
      <p style="line-height:1.7; margin-bottom:1rem;">${BANDHAVGARH_FORT.description}</p>
      <h4 style="margin-top:1.5rem; margin-bottom:0.5rem;">Fort Highlights:</h4>
  ...
  function renderTigerReserve() {
    ...
    container.innerHTML = `
      <h3 style="font-size:1.5rem; color:var(--bandhavgarh-accent-bright); margin-bottom:0.5rem;">🐅 ${TIGER_RESERVE.title}</h3>
      <p style="line-height:1.7; color:var(--bandhavgarh-muted);">${TIGER_RESERVE.description}</p>
    `;
  }
    ...
    WILDLIFE.forEach(function(w) {
      html += `
        <div class="glass-card wildlife-card">
          ...
          <div style="font-style:italic; font-size:0.85rem; color:var(--bandhavgarh-accent-bright); margin-bottom:0.8rem;">${w.scientificName}</div>
          ...
          <p style="font-size:0.9rem; color:var(--bandhavgarh-muted); line-height:1.5;">${w.description}</p>
        </div>`;
    });
    container.innerHTML = html;
  }
  ...
  function renderFlora() {
    const container = document.getElementById('flora-box');
    if (!container || typeof FLORA === 'undefined') return;

    const speciesHtml = FLORA.keySpecies.map(function(s) {
      return `
        <div class="glass-card flora-card">
          <h4 style="margin-bottom:0.5rem; color:var(--bandhavgarh-accent-bright);">${s.name}</h4>
          <p style="font-size:0.9rem; color:var(--bandhavgarh-muted); line-height:1.5;">${s.description}</p>
        </div>`;
    }).join('');

    container.innerHTML = `
      <p style="line-height:1.7; margin-bottom:1rem;">${FLORA.description}</p>
      <div class="flora-grid">${speciesHtml}</div>
    `;
  }
  ...
  function renderSafariInfo() {
    ...
    const zonesHtml = SAFARI_INFO.zones.map(z => `
      <div class="glass-card safari-zone-card">
        <h4 style="margin-bottom:0.5rem; color:var(--bandhavgarh-accent-bright);">${z.name}</h4>
        <p style="font-size:0.9rem; color:var(--bandhavgarh-muted); line-height:1.5;">${z.description}</p>
      </div>`).join('');
  ...
  function renderInteractiveMap() {
    ...
    MAP_HOTSPOTS.forEach(function(spot) {
      const icon = spot.category === 'gate' ? '🚪' : spot.category === 'heritage' ? '🏰' : '📍';
    ...
    container.querySelectorAll('.map-hotspot-pin').forEach(function(pin) {
      pin.addEventListener('click', function() {
        ...
        if (spot && infoPopup) {
          infoPopup.innerHTML = `<h4 style="color:var(--bandhavgarh-accent-bright); margin-bottom:0.3rem;">${spot.name}</h4><p style="font-size:0.85rem; color:var(--bandhavgarh-muted);">${spot.description}</p>`;
