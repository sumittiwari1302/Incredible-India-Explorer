/**
 * Sangeet Natak Akademi Award Explorer - Interactive Script
...
function activateTab(targetTab) {
    const tabs = document.querySelectorAll('.sna-tab');
    const sections = document.querySelectorAll('.sna-section');
    if (!tabs.length || !sections.length) return;

    tabs.forEach(t => t.classList.remove('active'));
    sections.forEach(s => s.classList.remove('active'));

    const tab = document.querySelector(`.sna-tab[data-tab="${targetTab}"]`);
...
function initTabNavigation() {
    document.querySelectorAll('.sna-tab').forEach(tab => {
...
function initScrollAnimations() {
    ...
    const targets = document.querySelectorAll('.eligibility-item, .category-card, .selection-step, .awardee-card, .gallery-item, .timeline-item, .sna-banner, .sna-content-card');
    targets.forEach(el => {
...
function initFooterTabs() {
    document.querySelectorAll('.sna-footer-tab').forEach(link => {
...
function initCardTilt() {
    ...
    cards.forEach(card => {
        card.addEventListener('mousemove', (e) => {
            ...
            const rx = (0.5 - py) * 8;
            const ry = (px - 0.5) * 10;
            card.classList.add('sna-card-tilt');
 ...
 */
function initTyping() {
    const el = document.querySelector('.sna-type[data-words]');
    if (!el) return;
...
function initHeroParallax() {
    const bg = document.querySelector('.sna-hero-bg');
    if (!bg) return;
...
function initCountUp() {
    const counters = document.querySelectorAll('.sna-count');
    if (!counters.length) return;

    const duration = 1600;
    const hero = document.querySelector('.sna-hero');
