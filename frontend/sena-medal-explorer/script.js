/**
 * Sena Medal Explorer - Interactive Script
...
function activateTab(targetTab) {
    ...
    const tab = document.querySelector(`.sm-tab[data-tab="${targetTab}"]`);
...
function initScrollAnimations() {
    ...
    const revealVariant = (el) => {
        if (el.classList.contains('gallery-item') || el.classList.contains('sm-timeline-item')) {
            const siblings = Array.from(el.parentNode.children).filter(s => s.classList.contains(el.classList[0]));
            return (siblings.indexOf(el) % 2 === 0) ? 'reveal-left' : 'reveal-right';
        }
        if (el.classList.contains('awardee-card') || el.classList.contains('type-card')) return 'reveal-scale';
        if (el.classList.contains('selection-step')) return 'reveal-right';
        if (el.classList.contains('eligibility-item')) return 'reveal-left';
        return 'reveal-up';
    };

    // Observe cards and sections with a staggered reveal delay
    const targets = document.querySelectorAll('.eligibility-item, .type-card, .selection-step, .awardee-card, .gallery-item, .sm-timeline-item, .sm-content-card');
    targets.forEach(el => {
...
function initFooterTabs() {
    document.querySelectorAll('.sm-footer-tab').forEach(link => {
...
function initCardTilt() {
    const cards = document.querySelectorAll('.type-card, .awardee-card, .gallery-item');
    ...
    cards.forEach(card => {
        card.addEventListener('mousemove', (e) => {
            ...
            const rx = (0.5 - py) * 8;
            const ry = (px - 0.5) * 10;
            card.classList.add('sm-card-tilt');
 ...
 */
function initTyping() {
    const el = document.querySelector('.sm-type[data-words]');
    if (!el) return;
...
function initHeroParallax() {
    const bg = document.querySelector('.sm-hero-bg');
    if (!bg) return;
...
function initCountUp() {
    const counters = document.querySelectorAll('.sm-count');
