...
function renderStats() {
    ...
    if (!grid || typeof MALABAR_FROG_INFO === 'undefined') return;

    grid.innerHTML = MALABAR_FROG_INFO.quickStats
        .map(
            stat => `
        <div class="stat-card">
...
function renderEcology() {
    ...
    if (overviewEl) overviewEl.innerHTML = `<strong>Overview:</strong> ${ECOLOGY_BEHAVIOUR.overview}`;
    if (distEl) distEl.innerHTML = `<strong>Geographic Distribution:</strong> ${ECOLOGY_BEHAVIOUR.distribution}`;
    if (habEl) habEl.innerHTML = `<strong>Canopy Habitat:</strong> ${ECOLOGY_BEHAVIOUR.habitat}`;
    if (morphEl) morphEl.innerHTML = `<strong>Morphology & Webbing:</strong> ${ECOLOGY_BEHAVIOUR.morphology}`;
    if (behEl) behEl.innerHTML = `<strong>Aerial Gliding Behaviour:</strong> ${ECOLOGY_BEHAVIOUR.behaviour}`;
    if (dietEl) dietEl.innerHTML = `<strong>Diet & Hunting:</strong> ${ECOLOGY_BEHAVIOUR.diet}`;
    if (reproEl) reproEl.innerHTML = `<strong>Monsoon Foam Nesting:</strong> ${ECOLOGY_BEHAVIOUR.reproduction}`;
    if (impEl) impEl.innerHTML = `<strong>Ecological Importance:</strong> ${ECOLOGY_BEHAVIOUR.ecologicalImportance}`;
    if (consEl) consEl.innerHTML = `<strong>Conservation Status:</strong> ${ECOLOGY_BEHAVIOUR.conservation}`;
}
