/**
 * Tulu Cinema Explorer - Interactive Logic
 * Handles modal popups for gallery items, smooth scrolling, and UI state management.
 */

document.addEventListener('DOMContentLoaded', () => {
  const modal = document.getElementById('tulu-cinema-modal');
  const modalClose = document.getElementById('tulu-cinema-modal-close');
  const modalTitle = document.getElementById('modal-title');
  const modalHeading = document.getElementById('modal-heading');
  const modalDescription = document.getElementById('modal-description');
  const galleryItems = document.querySelectorAll('.tulu-cinema-gallery-item');
