...
function readFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/cherai-beach', file),
        'utf-8'
    );
}

describe('Cherai Beach Kerala — Page Structure & Content (#3040)', () => {
    let html;
    ...
    it('contains hero section with badges, state and beach title', () => {
        expect(html).toContain('class="hero-section cherai-hero"');
        expect(html).toContain('<h1 id="hero-heading">');
        expect(html).toContain('Cherai Beach');
        expect(html).toContain('Kerala');
        expect(html).toContain('Vypin Island');
    });
    ...
    it('documents coastal setting, backwater connection, Munambam, and Pallipuram Fort', () => {
        expect(html).toContain('Munambam');
        expect(html).toContain('Pallipuram Fort');
        expect(html).toContain('Chinese Fishing Nets');
        expect(html).toContain('Arabian Sea');
        expect(html).toContain('Vembanad');
    });
