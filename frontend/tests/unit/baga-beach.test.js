/**
 * baga-beach.test.js
 * Unit tests for the Baga Beach (North Goa) page.
 * Validates required sections, key content, accessibility,
 * map reference, and gallery structure per issue #2954.
 */
...
function readExplorerFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/baga-beach', file),
        'utf-8'
    );
}

describe('Baga Beach — Page Structure', () => {
    let html;
    ...
    it('contains a hero section with page title and badges', () => {
        ...
        expect(html).toContain('Baga Beach');
    ...
    it('contains the key location and coastal culture details', () => {
        ...
        expect(html).toContain('Baga River');
    ...
    it('includes a map/location reference with Leaflet', () => {
        expect(html).toContain('baga-map');
    ...
    it('contains a gallery with lazy-loaded images and lightbox wiring', () => {
        expect(html).toContain('baga-gallery');
...
describe('Baga Beach — Styles and Scripts', () => {
