...
function readFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/mahabalipuram-beach', file),
        'utf-8'
    );
}

describe('Mahabalipuram Beach Tamil Nadu — Page Structure & Content (#3043)', () => {
    let html;
    ...
    it('contains hero section with badges, state and beach title', () => {
        expect(html).toContain('class="hero-section mahabalipuram-hero"');
        expect(html).toContain('<h1 id="hero-heading">');
        expect(html).toContain('Mahabalipuram Beach');
        expect(html).toContain('Tamil Nadu');
        expect(html).toContain('Chengalpattu');
    });

    it('contains all required requirement sections', () => {
        expect(html).toContain('id="location"');
        expect(html).toContain('id="heritage-connection"');
    ...
    it('documents UNESCO heritage, Pallava dynasty, Shore Temple, and Pancha Rathas', () => {
...
describe('Mahabalipuram Beach Tamil Nadu — Scripts & Styles', () => {
    it('style.css defines responsive cards, map, lightbox, and themes', () => {
        const css = readFile('style.css');
        expect(css).toContain('.mahabalipuram-hero');
