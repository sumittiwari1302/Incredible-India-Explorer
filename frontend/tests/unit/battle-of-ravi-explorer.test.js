/**
 * battle-of-ravi-explorer.test.js
 * Unit tests for the Battle of Ravi Explorer page.
...
function readExplorerFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/battle-of-ravi-explorer', file),
        'utf-8'
    );
}
...
describe('Battle of Ravi Explorer — Page Structure', () => {
    let html;
    ...
    it('contains a hero section with page title and kicker', () => {
        expect(html).toContain('class="ravi-hero"');
        expect(html).toContain('<h1>');
        expect(html).toContain('Battle of Ravi');
        expect(html).toContain('1306');
    });

    it('contains all required content sections from the issue', () => {
        const sections = ['overview', 'belligerents', 'commanders', 'outcome', 'timeline', 'references'];
        sections.forEach(id => {
    ...
    it('contains all required section topics', () => {
        ['Historical', 'Timeline', 'Belligerents', 'Commanders', 'Outcome', 'References'].forEach(topic => {
            expect(html).toContain(topic);
        });
    });

    it('contains the key historical and structural details', () => {
        expect(html).toContain('Malik Kafur');
        expect(html).toContain('Alauddin Khalji');
        expect(html).toContain('Kopek');
        expect(html).toContain('Malik Tughluq');
        expect(html).toContain('Duwa');
        expect(html).toContain('Chagatai');
        expect(html).toContain('Ravi River');
    });
...
describe('Battle of Ravi Explorer — Assets', () => {
    it('includes a non-empty stylesheet', () => {
        const css = readExplorerFile('style.css');
        expect(css.length).toBeGreaterThan(1000);
        expect(css).toContain('.ravi-hero');
        expect(css).toContain('.ravi-timeline');
        expect(css).toContain('.ravi-references');
    });
...
describe('Battle of Ravi — Landing Page Integration', () => {
    ...
    it('matches the existing featured card pattern (badge, heading, button)', () => {
        ...
        expect(card).toContain('Malik Kafur');
    });
});
