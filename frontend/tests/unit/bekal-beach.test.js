...
function readFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/bekal-beach', file),
        'utf-8'
    );
}

describe('Bekal Beach Kerala — Page Structure & Content (#3039)', () => {
    let html;
    ...
    it('contains hero section with badges, state and beach title', () => {
        expect(html).toContain('class="hero-section bekal-hero"');
        expect(html).toContain('<h1 id="hero-heading">');
        expect(html).toContain('Bekal Beach');
        expect(html).toContain('Kerala');
        expect(html).toContain('Kasaragod');
    });

    it('contains all required requirement sections', () => {
        expect(html).toContain('id="location"');
        expect(html).toContain('id="fort-connection"');
        expect(html).toContain('id="landscape"');
        expect(html).toContain('id="interactive-hub"');
        expect(html).toContain('id="gallery"');
    });

    it('documents the historic Bekal Fort connection in detail', () => {
        expect(html).toContain('Bekal Fort');
        expect(html).toContain('Shivappa Nayaka');
        expect(html).toContain('Tipu Sultan');
        expect(html).toContain('laterite');
        expect(html).toContain('keyhole');
    });
