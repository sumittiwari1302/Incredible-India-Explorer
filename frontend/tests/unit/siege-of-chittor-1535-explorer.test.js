/**
 * siege-of-chittor-1535-explorer.test.js
 * Unit tests for the Siege of Chittorgarh (1535) Explorer page.
...
function readExplorerFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/siege-of-chittor-1535-explorer', file),
        'utf-8'
    );
}
...
describe('Siege of Chittorgarh (1535) Explorer — Page Structure', () => {
    let html;
    ...
    it('contains a hero section with page title and kicker', () => {
        expect(html).toContain('class="chittor1535-hero"');
        expect(html).toContain('<h1>');
        expect(html).toContain('Siege of Chittorgarh');
        expect(html).toContain('8 March 1535');
    });

    it('contains all required content sections from the issue', () => {
        const sections = ['overview', 'belligerents', 'outcome', 'timeline', 'gallery', 'references'];
        sections.forEach(id => {
    ...
    it('contains all required section topics', () => {
        ['Historical', 'Timeline', 'Belligerents', 'Outcome', 'Jauhar', 'References'].forEach(topic => {
            expect(html).toContain(topic);
        });
    });

    it('contains the key historical and structural details', () => {
        expect(html).toContain('Rani Karnavati');
        expect(html).toContain('Bahadur Shah');
        expect(html).toContain('Rumi Khan');
        expect(html).toContain('Udai Singh');
        expect(html).toContain('jauhar');
        expect(html).toContain('saka');
        expect(html).toContain('Panna Dai');
    });
...
describe('Siege of Chittorgarh (1535) Explorer — Assets', () => {
    it('includes a non-empty stylesheet', () => {
        const css = readExplorerFile('style.css');
        expect(css.length).toBeGreaterThan(1000);
        expect(css).toContain('.chittor1535-hero');
        expect(css).toContain('.chittor1535-timeline');
        expect(css).toContain('.chittor1535-references');
    });
...
describe('Siege of Chittorgarh (1535) — Landing Page Integration', () => {
    ...
    it('matches the existing featured card pattern (badge, heading, button)', () => {
        ...
        expect(card).toContain('Second Jauhar of Chittor');
    });
});
