/**
 * anjuna-beach.test.js
 * Unit tests for the Anjuna Beach (North Goa) page.
 * Validates required sections, key content, accessibility,
 * map reference, gallery structure, and sources per issue #2957.
 */
...
function readExplorerFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/anjuna-beach', file),
        'utf-8'
    );
}

describe('Anjuna Beach — Page Structure', () => {
    let html;
    ...
    it('contains a hero section with page title and badges', () => {
        ...
        expect(html).toContain('Anjuna Beach');
        expect(html).toContain('North Goa');
        expect(html).toContain('Arabian Sea');
    });

    it('contains all required content sections from the issue', () => {
        const sections = ['geo-heading', 'overview-heading', 'culture-heading', 'marine-heading', 'panel-activities', 'panel-nearby', 'panel-visitor', 'gallery-heading'];
        sections.forEach(id => {
    ...
    it('contains all required section topics', () => {
        ['Location & Geographical Profile', 'Beach Characteristics', 'Cultural Highlights', 'Marine Environment', 'Activities', 'Nearby Attractions', 'Visitor Info', 'Gallery'].forEach(topic => {
    ...
    it('contains the key location and cultural details', () => {
        ...
        expect(html).toContain('Anjuna');
        expect(html).toContain('Hippie');
        expect(html).toContain('flea market');
        expect(html).toContain('Chapora');
        expect(html).toContain('Vagator');
    ...
    it('includes a map/location reference with Leaflet', () => {
        expect(html).toContain('anjuna-map');
    ...
    it('contains a gallery with lazy-loaded images and lightbox wiring', () => {
        expect(html).toContain('anjuna-gallery');
...
describe('Anjuna Beach — Styles and Scripts', () => {
    it('style.css defines a hero, map and responsive layout', () => {
        ...
        expect(css).toContain('.anjuna-map');
    });
