/**
 * palolem-beach.test.js
 * Unit tests for the Palolem Beach (South Goa) page.
 * Validates required sections, key content, accessibility,
 * map reference, gallery structure, and sources per issue #2958.
 */
...
function readExplorerFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/palolem-beach', file),
        'utf-8'
    );
}

describe('Palolem Beach — Page Structure', () => {
    let html;
    ...
    it('contains a hero section with page title and badges', () => {
        ...
        expect(html).toContain('Palolem Beach');
        expect(html).toContain('South Goa');
        expect(html).toContain('Arabian Sea');
    });

    it('contains all required content sections from the issue', () => {
        const sections = ['geo-heading', 'overview-heading', 'natural-heading', 'marine-heading', 'culture-heading', 'panel-activities', 'panel-nearby', 'panel-visitor', 'gallery-heading'];
        sections.forEach(id => {
    ...
    it('contains all required section topics', () => {
        ['Location & Geographical Profile', 'Beach Overview', 'Natural Features', 'Coastal Environment', 'Local Highlights', 'Activities', 'Nearby Attractions', 'Visitor Info', 'Gallery'].forEach(topic => {
            expect(html).toContain(topic);
        });
    });

    it('contains the key location and cultural details', () => {
        expect(html).toContain('Goa');
        expect(html).toContain('Canacona');
        expect(html).toContain('Palolem');
        expect(html).toContain('Crescent');
        expect(html).toContain('Canacona Island');
        expect(html).toContain('Patnem');
        expect(html).toContain('Agonda');
        expect(html).toContain('Butterfly');
        expect(html).toContain('Cotigao');
        expect(html).toContain('feni');
        expect(html).toContain('Silent Noise');
    });
    ...
    it('includes a map/location reference with Leaflet', () => {
        expect(html).toContain('palolem-map');
    ...
    it('contains a gallery with lazy-loaded images and lightbox wiring', () => {
        expect(html).toContain('palolem-gallery');
    ...
    it('includes a sources section as required by acceptance criteria', () => {
        expect(html).toContain('sources-heading');
        expect(html).toContain('Sources');
    });
});

describe('Palolem Beach — Styles and Scripts', () => {
    it('style.css defines a hero, map and responsive layout', () => {
        ...
        expect(css).toContain('.palolem-map');
    });
