/**
 * rashtriya-khel-protsahan-puruskar-explorer.test.js
 * Unit tests for the Rashtriya Khel Protsahan Puruskar Explorer page.
...
function readExplorerFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/rashtriya-khel-protsahan-puruskar-explorer', file),
        'utf-8'
    );
}
...
describe('Rashtriya Khel Protsahan Puruskar Explorer — Page Structure', () => {
    let html;
    ...
    it('contains a hero section with page title and kicker', () => {
        expect(html).toContain('class="rkpp-hero"');
        expect(html).toContain('<h1>');
        expect(html).toContain('Rashtriya Khel');
        expect(html).toContain('Protsahan');
    });

    it('contains all required content sections', () => {
        const sections = ['history', 'purpose', 'eligibility', 'categories', 'selection', 'recipients', 'timeline', 'gallery'];
        sections.forEach(id => {
    ...
    it('contains all required award categories as tab buttons', () => {
        ['Budding', 'Corporate Social Responsibility', 'Sportspersons', 'Sports for Development'].forEach(category => {
...
describe('Rashtriya Khel Protsahan Puruskar Explorer — Assets', () => {
...
describe('Rashtriya Khel Protsahan Puruskar — Landing Page Integration', () => {
    it('is listed as a card on the Awards of India Explorer landing page', () => {
        const index = readRootIndex();
        expect(index).toContain('Rashtriya Khel Protsahan Puruskar');
        expect(index).toContain("frontend/rashtriya-khel-protsahan-puruskar-explorer/index.html");
    });

    it('matches the existing award card pattern (icon, category, stats, button)', () => {
        const index = readRootIndex();
        const cardStart = index.indexOf("Rashtriya Khel Protsahan Puruskar Card");
