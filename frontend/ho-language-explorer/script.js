/**
 * Interactive Script for Ho Language Explorer (Kolhan & Mayurbhanj)
 */

document.addEventListener('DOMContentLoaded', () => {
    const data = window.HO_DATA;
    if (!data) {
        console.error("Ho dataset missing!");
        return;
    }
    ...
    if (themeBtn) {
        const savedTheme = localStorage.getItem('ho_theme') || 'dark';
        if (savedTheme === 'light') {
    ...
    function playAudio(text, lang = 'hi-IN') {
        ...
        const bestVoice = voices.find(v => v.lang.startsWith('hi') || v.lang.startsWith('or') || v.lang.startsWith('en-IN'));
        if (bestVoice) {
    ...
    function renderGreetings(filter = '') {
        ...
        greetingsGrid.innerHTML = list.map(g => `
            <div class="greeting-card">
                <div>
                    <span class="region-badge" style="background: rgba(217, 119, 6, 0.2); color: var(--accent-amber-bright); margin-bottom: 12px; display: inline-block;">${g.category}</span>
        ...
        cultureGrid.innerHTML = data.culturalHeritage.map(c => `
            <div class="culture-card">
                <div class="culture-card-body">
                    ...
                    <span class="region-badge" style="background: rgba(217, 119, 6, 0.2); color: var(--accent-amber-bright); margin-bottom: 12px; display: inline-block;">${c.category}</span>
        ...
        if (currentQ >= questions.length) {
            quizContainer.innerHTML = `
                <div style="text-align: center; padding: 24px;">
                    ...
                    <p style="font-size: 1.3rem; color: var(--accent-amber-bright); margin-bottom: 20px; font-weight: 700;">Your Score: ${score} / ${questions.length}</p>
                    <p style="color: var(--text-secondary); margin-bottom: 28px; max-width: 500px; margin-left: auto; margin-right: auto;">
                        ${score === questions.length ? 'Johar! You have demonstrated exceptional knowledge of the Ho language, Warang Citi script, and Kolhan heritage!' : 'Great effort! Revisit the vocabulary visualizer and script showcase to master every concept.'}
                    </p>
