...
(function () {
    ...
    function setupBookmark() {
        ...
        if (!btn) return;
        const id = 'festival-onam';
        const updateBtn = () => { btn.textContent = window.Journey && window.Journey.isSaved(id) ? '✅ Saved to Journey' : '🔖 Bookmark to My Journey'; };
        updateBtn();
        btn.addEventListener('click', () => {
            if (window.Journey) {
                window.Journey.toggle({ id, explorerPage: 'frontend/onam-festival/index.html', title: 'Onam Festival', thumbnail: 'https://placehold.co/100/FFB703/1A2421', category: 'festivals' });
                updateBtn();
            }
        });
    }
    ...
    function setupJourneyIntegration() {
        if (window.Journey && typeof window.Journey.registerSearchItems === 'function') {
            window.Journey.registerSearchItems('frontend/onam-festival/index.html', [
                { id: 'festival-onam', title: 'Onam', description: 'Kerala\'s grand harvest festival and homecoming of Mahabali.', link: '#' }
            ]);
        }
    }
