/**
 * Sanchi Stupa Explorer Interactive Script
 * Handles Journey bookmarking, lightbox modal, and scroll-to-top.
 */

document.addEventListener('DOMContentLoaded', () => {
  // Lightbox Modal Functionality
  const modal = document.getElementById('sanchi-modal');
  const modalImg = document.getElementById('sanchi-modal-img');
  const modalCaption = document.getElementById('sanchi-modal-caption');
  const modalOverlay = document.getElementById('sanchi-modal-overlay');
  const modalClose = document.getElementById('sanchi-modal-close');
  const galleryItems = document.querySelectorAll('.gallery-item');

  function openModal(imgSrc, captionText) {
    if (!modal || !modalImg) return;
    modalImg.src = imgSrc;
    modalImg.alt = captionText || 'Sanchi Stupa Gallery Image';
  ...
  if (bookmarkBtn) {
    const bookmarkId = bookmarkBtn.getAttribute('data-bookmark-id') || 'sanchi-stupa-explorer-main';
