// mewari.js — Mewari Language Explorer logic
...
(function () {
    function speak(text) {
        ...
        // No dedicated Mewari voice exists in most browsers; prefer an
    ...
    function renderStats() {
        ...
        if (!grid) return;
        grid.innerHTML = MEWARI_STATS.map(
            (s) => `
            <div class="stat-item">
    ...
    function renderScript() {
        document.getElementById("script-intro").textContent = MEWARI_SCRIPT.intro;
        const grid = document.getElementById("script-facts-grid");
        grid.innerHTML = MEWARI_SCRIPT.facts
            .map(
                (f) => `
    ...
    function renderWords() {
        ...
        if (!grid) return;
        grid.innerHTML = MEWARI_WORDS.map(
            (w, i) => `
            <div class="word-card">
        ...
        grid.querySelectorAll(".word-pronounce-btn").forEach((btn) => {
            btn.addEventListener("click", () => {
                const w = MEWARI_WORDS[Number(btn.dataset.index)];
                speak(w.deva);
            });
        });
    }
    ...
    function renderCulture() {
        ...
        if (!grid) return;
        grid.innerHTML = MEWARI_CULTURE.map(
            (c) => `
    ...
    function renderReferences() {
        ...
        if (!el) return;
        el.innerHTML = MEWARI_REFERENCES.map(
