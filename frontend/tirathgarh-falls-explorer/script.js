...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderHeightChart() {
        ...
        chart.innerHTML = HEIGHT_COMPARISONS.map(item => {
            ...
            const isTirathgarh = item.name.includes('Tirathgarh');
            return `
                <div class="height-bar-container">
                    <div class="height-bar ${isTirathgarh ? 'highlight-bar' : ''}" style="height: ${pct}%; background: ${isTirathgarh ? 'var(--tirath-teal)' : '#aaa'};" title="${item.note}"></div>
                    <span class="${isTirathgarh ? 'highlight-text' : ''}">${item.name} (${item.heightMeters}m)</span>
                </div>
            `;
        }).join('');
    }
    ...
    function renderAttractions() {
        ...
        grid.innerHTML = NEARBY_ATTRACTIONS.map(a => `
            <div class="attraction-card glass-card">
                <h3>📍 ${a.name}</h3>
                <span class="dist-tag">${a.distance} from Tirathgarh Falls</span>
                <p>${a.description}</p>
            </div>
        `).join('');
    }
    ...
    /* ================================================================
       ...
       ================================================================ */
    function initMaps() {
        if (typeof L === 'undefined' || typeof TIRATHGARH_COORDS === 'undefined') return;

        // Main location map
        const mapContainer = document.getElementById('tirath-map');
        if (mapContainer) {
            const map = L.map('tirath-map', {
                center: TIRATHGARH_COORDS,
                zoom: 12,
            ...
            L.marker(TIRATHGARH_COORDS).addTo(map).bindPopup(`
                <div style="text-align: center;">
                    <strong>Tirathgarh Falls</strong><br>
                    Kanger Valley National Park, Bastar<br>
                    <em>On the Munga Bahar / Kanger River</em>
                </div>
            `).openPopup();
        ...
        // Forest ecosystem map (park boundary context)
        const forestContainer = document.getElementById('forest-map');
        if (forestContainer) {
            const forestMap = L.map('forest-map', {
                center: [18.94, 81.90],
                zoom: 10,
                scrollWheelZoom: false
            });
            ...
            L.marker(TIRATHGARH_COORDS).addTo(forestMap).bindPopup('<strong>Tirathgarh Falls</strong><br>Western edge of Kanger Valley National Park');
            L.circle([18.94, 81.90], { radius: 17000, color: '#38a169', weight: 2, fillOpacity: 0.08 })
                .addTo(forestMap)
                .bindPopup('Approximate extent of Kanger Valley National Park');

            forestMap.on('click', () => { if (!forestMap.scrollWheelZoom.enabled()) forestMap.scrollWheelZoom.enable(); });
            forestMap.on('mouseout', () => forestMap.scrollWheelZoom.disable());
        }
        ...
        if (attrContainer && typeof NEARBY_ATTRACTIONS !== 'undefined') {
            ...
            L.marker(TIRATHGARH_COORDS).addTo(attrMap).bindPopup('<strong>Tirathgarh Falls</strong>');
