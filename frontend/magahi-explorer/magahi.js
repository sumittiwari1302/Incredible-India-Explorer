// magahi.js — Magahi Language Explorer logic
...
(function () {
    function speak(text) {
        ...
        // Magahi has no dedicated browser voice; hi-IN is the closest
    ...
    function renderStats() {
        ...
        if (!grid) return;
        grid.innerHTML = MAGAHI_STATS.map(
            (s) => `
            <div class="stat-item">
    ...
    function renderGreeting() {
        ...
        document.getElementById("folk-greeting").innerHTML = `
            <span class="folk-greeting-label">Traditional / Folk Variant</span>
    ...
    function renderScript() {
        document.getElementById("script-intro").textContent = MAGAHI_SCRIPT.intro;
        const grid = document.getElementById("script-facts-grid");
        grid.innerHTML = MAGAHI_SCRIPT.facts
            .map(
                (f) => `
    ...
    function renderWords() {
        ...
        if (!grid) return;
        grid.innerHTML = MAGAHI_WORDS.map(
            (w, i) => `
            <div class="word-card">
        ...
        grid.querySelectorAll(".word-pronounce-btn").forEach((btn) => {
            btn.addEventListener("click", () => {
                const w = MAGAHI_WORDS[Number(btn.dataset.index)];
                speak(w.devanagari);
            });
        });
    }
    ...
    function renderCulture() {
        ...
        if (!grid) return;
        grid.innerHTML = MAGAHI_CULTURE.map(
            (c) => `
    ...
    function renderReferences() {
        ...
        if (!el) return;
        el.innerHTML = MAGAHI_REFERENCES.map(
