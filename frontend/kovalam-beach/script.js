/* ==========================================================================
   Kovalam Explorer Logic
   Handles tabs, lightbox, and Journey API integration.
   ========================================================================== */
(function () {
    'use strict';

    function init() {
        renderFeatureGrid('beaches-grid', kovalamBeaches);
        renderFeatureGrid('activities-grid', kovalamActivities);
        renderKovalamGallery();
        setupTabs();
        setupThemeToggle();
        setupBookmark();
        setupLightbox();
    ...
    function setupBookmark() {
        ...
        if (!btn) return;
        const id = 'beach-kovalam';
        const updateBtn = () => { btn.textContent = window.Journey && window.Journey.isSaved(id) ? '✅ Saved to Journey' : '🔖 Bookmark to My Journey'; };
        updateBtn();
        btn.addEventListener('click', () => {
            if (window.Journey) {
                window.Journey.toggle({ id, explorerPage: 'frontend/kovalam-beach/index.html', title: 'Kovalam Beach', thumbnail: 'https://placehold.co/100/00B4D8/fff', category: 'beaches' });
                updateBtn();
            }
        });
    }
    ...
    function setupJourneyIntegration() {
        if (window.Journey && typeof window.Journey.registerSearchItems === 'function') {
            window.Journey.registerSearchItems('frontend/kovalam-beach/index.html', [
                { id: 'beach-kovalam', title: 'Kovalam Beach', description: 'Kerala\'s crescent-shaped coastal paradise and Ayurvedic hub.', link: '#' }
            ]);
        }
    }
