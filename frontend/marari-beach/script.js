/**
 * Marari Beach Profile Interactive Features
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initMap() {
        if (!window.L) return;
        const mapEl = document.getElementById('marari-map');
        if (!mapEl) return;

        const map = L.map('marari-map', { scrollWheelZoom: false }).setView([9.6015, 76.2995], 13);
        ...
        L.marker([9.6015, 76.2995])
            .addTo(map)
            .bindPopup('<strong>Marari Beach</strong><br>Mararikulam, Alappuzha, Kerala')
            .openPopup();

        const nearby = [
            { name: 'Alappuzha Lighthouse & Beach', coords: [9.4925, 76.3175], note: '~15 km South' },
            { name: 'Vembanad Lake Backwaters', coords: [9.5916, 76.4365], note: '~14 km East' },
            { name: 'Arthunkal St. Andrew\'s Basilica', coords: [9.6865, 76.2934], note: '~8 km North' }
        ];
    ...
    galleryItems.forEach(item => {
        const openLightbox = () => {
            ...
            if (img && modal && modalImg) {
                ...
                modalTitle.textContent = title ? title.textContent : 'Marari Beach Visual';
