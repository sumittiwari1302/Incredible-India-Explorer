/**
 * battle-of-haldighati-explorer.test.js
 * Unit tests for the Battle of Haldighati Explorer page.
...
function readExplorerFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/battle-of-haldighati-explorer', file),
        'utf-8'
    );
}
...
describe('Battle of Haldighati Explorer — Page Structure', () => {
    let html;
    ...
    it('contains a hero section with page title and kicker', () => {
        expect(html).toContain('class="haldi-hero"');
        expect(html).toContain('<h1>');
        expect(html).toContain('Battle of Haldighati');
        expect(html).toContain('18 June 1576');
    });

    it('contains all required content sections from the issue', () => {
        const sections = ['overview', 'figures', 'strategy', 'outcome', 'legacy', 'timeline', 'references'];
        sections.forEach(id => {
    ...
    it('contains all required section topics', () => {
        ['Historical', 'Commanders', 'Military Strategy', 'Outcome', 'Legacy', 'Timeline', 'References'].forEach(topic => {
            expect(html).toContain(topic);
        });
    });

    it('contains the key historical and structural details', () => {
        expect(html).toContain('Maharana Pratap');
        expect(html).toContain('Man Singh');
        expect(html).toContain('Chetak');
        expect(html).toContain('Haldighati');
        expect(html).toContain('Bhamashah');
        expect(html).toContain('Hakim Khan Sur');
        expect(html).toContain('Akbar');
        expect(html).toContain('Jhala Maan');
    });
...
describe('Battle of Haldighati Explorer — Assets', () => {
    it('includes a non-empty stylesheet', () => {
        const css = readExplorerFile('style.css');
        expect(css.length).toBeGreaterThan(1000);
        expect(css).toContain('.haldi-hero');
        expect(css).toContain('.haldi-timeline');
        expect(css).toContain('.haldi-references');
    });
