/**
 * battle-of-jalore-explorer.test.js
 * Unit tests for the Battle of Jalore Explorer page.
...
function readExplorerFile(file) {
    return readFileSync(
        resolve(__dirname, '../../frontend/battle-of-jalore-explorer', file),
        'utf-8'
    );
}
...
describe('Battle of Jalore Explorer — Page Structure', () => {
    let html;
    ...
    it('contains a hero section with page title and kicker', () => {
        expect(html).toContain('class="jalore-hero"');
        expect(html).toContain('<h1>');
        expect(html).toContain('Battle of Jalore');
        expect(html).toContain('1311');
    });

    it('contains all required content sections from the issue', () => {
        const sections = ['overview', 'timeline', 'figures', 'outcome', 'legacy', 'references'];
        sections.forEach(id => {
    ...
    it('contains all required section topics', () => {
        ['Historical Overview', 'Timeline', 'Key Figures', 'Outcome', 'Legacy', 'References'].forEach(topic => {
    ...
    it('contains the key historical and structural details', () => {
        ...
        expect(html).toContain('Kanhadadeva');
        expect(html).toContain('Viramadeva');
        expect(html).toContain('Malik Kamaluddin');
        expect(html).toContain('Chauhan');
        expect(html).toContain('Swarnagiri');
...
describe('Battle of Jalore Explorer — Assets', () => {
    it('includes a non-empty stylesheet', () => {
        const css = readExplorerFile('style.css');
        expect(css.length).toBeGreaterThan(1000);
        expect(css).toContain('.jalore-hero');
        expect(css).toContain('.jalore-timeline');
        expect(css).toContain('.jalore-references');
    });
...
describe('Battle of Jalore — Landing Page Integration', () => {
    ...
    it('matches the existing featured card pattern (badge, heading, button)', () => {
        ...
        expect(card).toContain('Kanhadadeva');
    });
});
